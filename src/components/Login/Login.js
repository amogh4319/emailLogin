...
// },[enteredEmail,enteredPassword])
  ...
  const emailChangeHandler = (event) => {
    ...
    setFormIsValid(
             emailState.isValid.includes('@') && passwordState.ispassValid.trim().length > 6
           );
   
  };

  const passwordChangeHandler = (event) => {
    dispatchPassword({type:'PAASWORD_INPUT',val:event.target.value})

    setFormIsValid(
      passwordState.ispassValid.trim().length > 6 && emailState.isValid.includes('@')
    );
  };
